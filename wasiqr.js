...
router.get('/', async (req, res) => {
	const id = makeid();
	async function ARIMA_MD_QR_CODE() {
		const {
			state,
			saveCreds
		...
		try {
			...
			Qr_Code_By_Arima_Tech.ev.on("connection.update", async (s) => {
				...
				if (connection == "open") {
				   ...
				   let session = await Qr_Code_By_Arima_Tech.sendMessage(Qr_Code_By_Arima_Tech.user.id, { text: '' + b64data });
	
				   let ARIMA_MD_TEXT = `
*_Session Connected By V-ARIMA*
*_Made With 🤍_*
______________________________________
╔════◇
║ *『 AMAZING YOU'VE CHOSEN V-ARIMA 』*
...
_Don't Forget To Give Star To My Repo_`
	 await Qr_Code_By_Arima_Tech.sendMessage(Qr_Code_By_Arima_Tech.user.id,{text:V-ARIMA_MD_TEXT},{quoted:session})
